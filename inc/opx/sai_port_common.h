...
typedef struct _sai_port_attr_info_t
{
    ...
    /** Ingress Filtering - Drop Frames with Unknown VLANs (default to FALSE) */
    ...
    /* FDB Learning mode (default to SAI_PORT_FDB_LEARNING_MODE_HW) */
    sai_port_fdb_learning_mode_t      fdb_learning;
    ...
    uint_t                            max_learned_address;

    /** Action for packets with unknown source mac address
     * when FDB learning limit is reached. (default to SAI_PACKET_ACTION_DROP) */
    sai_packet_action_t               fdb_learn_limit_violation;
