...
void sai_fib_dump_help (void)
{
    ...
    SAI_DEBUG ("  void sai_fib_dump_neighbor_entry (uint_t af_family, ");
    ...
    SAI_DEBUG ("       uint_t af, char *ip_str, uint_t prefix_len)");
    ...
    SAI_DEBUG ("  sai_object_id_t vr, uint_t af, char *ip_str, uint_t prefix_len)");
    SAI_DEBUG ("  void sai_fib_dump_dep_encap_nh_list_for_nexthop (uint_t af, ");
    SAI_DEBUG ("       const char *ip_str, sai_object_id_t rif_id)");
...
void sai_fib_dump_route_entry (sai_object_id_t vrf, uint_t af_family, char *ip_str,
                               uint_t prefix_len)
{
    ...
    if (af_family == 4) {
        ...
        SAI_DEBUG ("af_family must be 4 or 6. af_family %d is not valid.",
                   af_family);
...
void sai_fib_dump_neighbor_entry (uint_t af_family, const char *ip_str,
                                  sai_object_id_t rif_id)
{
    ...
    if (af_family == 4) {
        ...
        SAI_DEBUG ("af_family must be 4 or 6. af_family %d is not "
                   "valid.", af_family);
...
void sai_fib_dump_dep_encap_nh_list_for_route (sai_object_id_t vrf, uint_t af,
                                               char *ip_str, uint_t prefix_len)
{
    ...
    if (af == 4) {
        ...
        SAI_DEBUG ("af must be 4 or 6. Invalid %d value passed.", af);
        return;
    }
...
void sai_fib_dump_dep_encap_nh_list_for_nexthop (uint_t af, const char *ip_str,
                                                 sai_object_id_t rif_id)
{
